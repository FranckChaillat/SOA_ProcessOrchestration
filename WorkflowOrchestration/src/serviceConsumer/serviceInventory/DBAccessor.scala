...
object DBAccessor {
  ...
  var conn = loadConn()
    ...
    private def getRes(rs : ResultSet,
      ...
      if(rs.next){
          ...
          for(i<- 1.until(mtd.getColumnCount+1))m= m.+(mtd.getColumnLabel(i) ->   Class.forName(mtd.getColumnClassName(i)).cast(rs. getObject(i)))
          res.+=(m)
      }
      else return res
  ...
  def execQuery(proc: String, params: Seq[Any]) : Seq[Map[String, Any]] = {
    ...
    for((j,i)<-params.view.zipWithIndex)  stm.setObject(i+1, j.asInstanceOf[String])
